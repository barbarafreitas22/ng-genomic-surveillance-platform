...
""")
    ...
    **De novo Assembly**
    ...
    - **plasmidSPAdes** — parallel plasmid-specific assembly for detection of plasmid-borne resistance genes
        ...
        if db and _asm_proj and st.session_state.get("_asm_loaded_project") != _asm_proj:
            ...
            _asm_from_db = {
                sid: {
                    ...
                    "plasmid_contigs_path": d["assembly"].get("plasmid_contigs_path"),
                }
        ...
        if _n_afin < _n_atot:
            ...
            for _sid_j, _ares in _asm_collected.items():
                ...
                if _cpath and "error" not in _ares and db:
                    ...
                    _pc = _ares.get("plasmid_contigs_path")
                    if _pc:
                        _amr_payload["plasmid_contigs"] = _pc
                    _amr_job_ids.append(
    ...
    if _asm_results:
        ...
        # Plasmid assembly results
        _plasmid_paths = {
            _sid: Path(_res["plasmid_contigs_path"])
            for _sid, _res in _asm_results.items()
            if _res.get("plasmid_contigs_path")
        }
        _no_plasmid = [
            _sid for _sid, _res in _asm_results.items()
            if "error" not in _res and not _res.get("plasmid_contigs_path")
        ]
        if _plasmid_paths or _no_plasmid:
            st.markdown("---")
            st.markdown("##### Plasmid Assembly (plasmidSPAdes)")
            if _plasmid_paths:
                st.success(
                    f"Plasmid contigs detected in {len(_plasmid_paths)} sample(s): "
                    + ", ".join(f"**{s}**" for s in _plasmid_paths)
                )
                _pcols = st.columns(min(len(_plasmid_paths), 4))
                for _i, (_sid, _path) in enumerate(_plasmid_paths.items()):
                    _pcols[_i % 4].download_button(
                        f"⬇ {_sid} plasmids",
                        data=_path.read_bytes(),
                        file_name=f"{_sid}_plasmid_contigs.fasta",
                        mime="application/octet-stream",
                        key=f"plasm_dl_{_sid}",
                    )
            if _no_plasmid:
                st.info(
                    f"No plasmids detected in: {', '.join(_no_plasmid)}"
                )
