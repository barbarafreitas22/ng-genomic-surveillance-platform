...
def _render_genome_overview(contigs_path: str, amr_result: dict, sample_label: str = "") -> None:
    ...
    with st.expander("Genome overview", expanded=False):
        ...
        with _t2:
            ...
            for gene, muts in amr_result.chromosomal.items():
            ...
            for gene, status in amr_result.plasmid.items():
            ...
            for gene, muts in amr_result.synonymous_variants.items():
                if not muts or gene not in gene_map:
                    continue
                gm = gene_map[gene]
                for mut in muts:
            ...
            for gname, gm in gene_map.items():
                chrom_muts   = list(amr_result.chromosomal.get(gname) or [])
                plasm_status = amr_result.plasmid.get(gname)
