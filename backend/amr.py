...
_MTRR_PROMOTER_MOSAIC_THRESHOLDS: dict[str, tuple[float, float]] = {
    "mtrR_promoter_mosaic_1": (99.0, 0.98),
    "mtrR_promoter_mosaic_2": (95.0, 0.98),
    "mtrR_promoter_mosaic_3": (98.5, 0.98),
}
...
def detect_mtr_mosaics(contigs: Path) -> dict:
    ...
    targets: list[tuple[str, float, float]] = (
        [(name, pid, cov) for name, (pid, cov) in _MTRR_PROMOTER_MOSAIC_THRESHOLDS.items()]
...
def run_amr_variant_calling(sample_id: str, contigs: Path):
    ...
    plasmid, dbg_plasm                 = _call_plasmid_genes(contigs)
