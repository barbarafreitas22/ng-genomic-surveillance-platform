...
def _process_single_sample(project_name: str, sample_id: str, r1: Path, r2,
                           run_plasmid: bool = False) -> dict:
    ...
    try:
        ...
        if asm.plasmid_contigs:
            result["plasmid_contigs_path"] = asm.plasmid_contigs
    except Exception as e:
    ...
    if result["contigs_path"]:
        try:
            amr_out = run_amr_variant_calling(
                ...
                plasmid_contigs=result.get("plasmid_contigs_path"),
            )
            result["amr"] = amr_out
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if sr["contigs_path"]:
                ...
                _plasmid_str = str(sr["plasmid_contigs_path"]) if sr.get("plasmid_contigs_path") else None
                save_assembly(project_name, sample_id, str(sr["contigs_path"]), _stats,
                              plasmid_contigs_path=_plasmid_str)
