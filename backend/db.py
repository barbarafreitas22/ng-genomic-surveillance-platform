...
def save_assembly(project_name: str, sample_id: str, contigs_path: str, stats,
                  plasmid_contigs_path: str | None = None) -> None:
    now = _now()
    ...
    with _conn(project_name) as con:
        ...
        con.execute("""
            INSERT OR REPLACE INTO assembly_results
                (sample_id, contigs_path, plasmid_contigs_path,
        ...
        for row in con.execute("SELECT * FROM assembly_results"):
            ...
            if sid in samples:
                samples[sid]["assembly"] = {
                    ...
                    "plasmid_contigs_path": row["plasmid_contigs_path"],
                    "stats": {
